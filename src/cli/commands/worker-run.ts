...
function buildGeminiWorkerPrompt(input: {
  ...
  return [
    `You are oh-my-antigravity worker ${input.workerName} for team ${input.teamName}.`,
...
export async function executeWorkerRunCommand(
  ...
  if (!teamName || !workerName) {
    io.stderr('[oh-my-antigravity] worker run: --team and --worker are required');
    return { exitCode: 2 };
  }

  io.stdout(`[oh-my-antigravity] worker ${workerName} starting for team ${teamName}`);
  ...
  try {
    ...
    if (contextContent) {
      io.stdout(`[oh-my-antigravity] team context loaded (${contextContent.length} chars)`);
    }

    io.stdout(`[oh-my-antigravity] worker ${workerName} executing task for team ${teamName}`);
    ...
    if (preClaimedTaskId && preClaimedToken) {
      ...
      await controlPlane
        ...
        .catch((err) => {
          io.stderr(
            `[oh-my-antigravity] failed to transition task status: ${(err as Error).message}`,
          );
        });
    }
    ...
    io.stderr(`[oh-my-antigravity] worker ${workerName} failed: ${doneError}`);
    exitCode = 1;
    await stateStore
      .writeWorkerStatus(teamName, workerName, {
        state: 'failed',
    ...
    await stateStore
      ...
      .catch((err) => {
        io.stderr(`[oh-my-antigravity] failed to write done signal: ${(err as Error).message}`);
      });
  ...
  io.stdout(`[oh-my-antigravity] worker ${workerName} done`);
  return { exitCode };
}
