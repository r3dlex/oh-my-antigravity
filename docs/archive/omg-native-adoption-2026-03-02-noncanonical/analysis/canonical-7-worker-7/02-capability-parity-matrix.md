...
|---|---|---|---|---|---|
<<<<<<< HEAD
| Lifecycle command surface | `team run` centered; operator lifecycle incomplete | First-class lifecycle operations | Implement `team status`, `team resume`, `team shutdown` with deterministic behavior | `src/cli/index.ts`, `src/cli/commands/*`, `extensions/oh-my-antigravity/commands/team/*` | command contract tests + UX docs parity |
=======
