...
| `/handoff` | 映射到 writer 角色以生成交接产物 | `omg team run --task "/subagents /handoff summarize the release state" --workers 1` |
| `--madmax` | 启动 Gemini 时将交互式启动扩展为 `--yolo --sandbox=none` | `omg --madmax` |
...
| `omg skill` | 列出或输出可复用技能提示 | `omg skill list` |
