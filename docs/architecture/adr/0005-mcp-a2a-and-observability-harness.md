...
`oh-my-antigravity` had no observability surface and no `.ai/mcp/` governance
surface, even though it already ships an MCP tools server
(`gemini-extension.json` -> `mcpServers.omg_cli_tools`, command
`oh-my-antigravity tools serve`).
...
- Generate an **observability** surface: `.ai/observability/conventions.md`
  ...
  and checklist exist.
- Promote **MCP/A2A** to a governed surface: `.ai/mcp/registry.json` (MCP-server
  registry stub — `status: "stub"`, no live endpoint resolved; records the repo's
  own `omg_cli_tools` server alongside generic stubs) and `.ai/mcp/a2a-handoff.md`
  (A2A cross-agent handoff convention). Generation makes no network or model call.
