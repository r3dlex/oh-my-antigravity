...
This generated workflow is the shared source of truth for mandatory and optional AI-SDLC initialization steps in `oh-my-antigravity` (topology: standalone, depth 0).
...
- **skill-modernization** - enabled when the target repo owns a skill catalog; status: `not-applicable-not-a-catalog` (this is a node CLI tool / Gemini extension, not a skills catalog).

## Entry surface links

Generated `AGENTS.md` and `README.md` must link to this workflow doc and `.ai/workflows/repo-workflow.json`. `CLAUDE.md` is a thin pointer to `AGENTS.md` (ADR-0004). `GEMINI.md` is the Gemini/Antigravity extension `contextFileName` product surface and remains content-bearing (documented exception, ADR-0004); it is not a generated entry surface for this workflow.
